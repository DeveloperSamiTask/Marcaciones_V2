...
export default function UploadSuspension({ suspensionId } : {suspensionId : number}) {
    const fileInputRef = useRef<HTMLInputElement>(null);
    const { data, setData, post, processing, reset, errors, clearErrors } = useForm<Required<{sustento: File | null }>>({ sustento: null });

    const uploadSuspension: FormEventHandler = (e) => {
        e.preventDefault();

        post(route('suspensiones.upload', suspensionId), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                ...
                <DialogDescription>
                    Este comprobante es un sustento para validar la suspension
                </DialogDescription>
                <form className="space-y-6" onSubmit={uploadSuspension}>
